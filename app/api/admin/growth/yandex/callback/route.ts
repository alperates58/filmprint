...
export async function GET(request: Request) {
  ...
  if (errorParam) {
    ...
    return NextResponse.redirect(`${baseUrl}/admin/growth?tab=yandex&error_code=${encodeURIComponent(errorParam)}&error=${encodeURIComponent(combinedError)}`);
  }

  if (!code || !state) {
    return NextResponse.redirect(`${baseUrl}/admin/growth?tab=yandex&error_code=missing_params&error=${encodeURIComponent("Eksik Yandex yetkilendirme parametreleri")}`);
  }

  const isValidState = verifyYandexGrowthState(state);
  if (!isValidState) {
    return NextResponse.redirect(`${baseUrl}/admin/growth?tab=yandex&error_code=invalid_state&error=${encodeURIComponent("Geçersiz veya süresi dolmuş OAuth state")}`);
  }
  ...
  try {
    ...
    return NextResponse.redirect(`${baseUrl}/admin/growth?tab=yandex&error_code=token_exchange_failed&error=${encodeURIComponent(err?.message || "Yandex bağlantısı başarısız oldu")}`);
  }
}
