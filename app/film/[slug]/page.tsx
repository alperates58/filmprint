  ...
  if (!director || !cast.length || trailer === undefined) {
    try {
      ...
      const updatedMeta = {
        ...
        trailer,
        runtime,
        overview: details.localization?.overview || overview,
      };
      ...
      db.movie
        ...
        .catch((err) => console.error("[Film Page Metadata Persistence Error]:", err));
    } catch {
      // Fallback
    }
  }
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3 sm:gap-4">
              {movie.cast.map((actor: any, idx: number) => {
                ...
                return (
                  <Link
                    key={idx}
                    href={`/arama?oyuncu=${encodeURIComponent(actor.name)}`}
                    ...
                    title={`${actor.name} filmlerini ara`}
                  >
